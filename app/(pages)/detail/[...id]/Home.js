...
function Home() {
  ...
  const novelDetailData = (sort) => {
    ...
    if (localStorage.getItem("token")) {
      form = `id=${param?.id[param?.id?.length - 1]
        }&userId=${userid}&guestId=${guestTabId}&chapterSort=${sort ? sort : "DESC"
        }`;
    } else {
      form = `id=${param?.id[param?.id?.length - 1]
  ...
  const handleReading = (ak1, para2, para3) => {
    setLoading(true);
    console.log(ak1, para2, para3);
    const chapterPath = ak1 ? para2.trim() : para3.trim();
    router.push(`/chapter/${chapterPath}`);
    setTimeout(() => {
      setLoading(false);
    }, 3000);
  };
  const handleReadingStr = (id) => {
    console.log(id);
    setLoading(true);
                      ...
                      (detailData?.readingStatus?.length > 0 ? (
                        <a
                          onClick={() => handleReading(
                            ...
                            currentChapterStatus[0]?.chapterId.trim(),
                            currentChapterStatus?._id.trim()
                          )}
                        >
                        ...
                        <a
                          onClick={() => handleReadingStr(detailData?.chapter[detailData?.chapter.length - 1]?._id)}
                        >
                          <button className="border px-14 py-2 slideBtn sliderRight">
                            START READING
                          </button>
